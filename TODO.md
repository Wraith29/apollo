...
- Commands
    ...
    - Recommend Album
        - [x] When an artist is selected, if the album selection fails for any reason, go back to Artist selection.
        - [x] Add a check for whether the command is running on Mobile or Desktop and adjust styling accordingly.
            - [x] Desktop: Render with the album cover in a column on the left, then a column with details (artist + album names, external links)
            - [x] Mobile: Render everything as a single column (Modal header is the Artist name?). Then render album name and links below
    - Recommend Artist
